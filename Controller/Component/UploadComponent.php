...
class UploadComponent extends Component
{
	...
	public function facebook_image($facebook_id, $destination_dir)
	{
		...
		$fb_img = @file_get_contents('https://graph.facebook.com/'.$facebook_id.'/picture?type=large');
		$this->filename = $file_name = $facebook_id.uniqid().'.jpg';		
		$source_path = USER_AVTAR_TMP . $file_name;		
		...
		$dest_path.$this->filename ;

		if (copy($source_path , $dest_path.$this->filename ))
		{
			//echo 'copied';
			@unlink($source_path);
		}
	...
	public function gallery($upload_path , $upload_directory, $file_name , $file_upload_name )
	{
		//list($width, $height, $type, $attr) = getimagesize($upload_path );	
						
		$config['upload_path'] = $upload_directory ;
		$config['file_name'] = $file_name ;
		$config['allowed_types'] = 'gif|jpg|png|jpeg';
		$config['max_size'] = '2400';					
		$this->initializes($config);					
		
		if ($this->do_upload($file_upload_name))
		{			
			$imgdata_arr = $this->data();	
